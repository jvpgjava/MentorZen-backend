...
public class EssayController {
        ...
        @PostMapping
        @Operation(summary = "Criar nova redação", description = "Cria uma nova redação em rascunho para o usuário autenticado")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "201", description = "Redação criada com sucesso", content = @Content(schema = @Schema(implementation = EssayResponse.class))),
                        @ApiResponse(responseCode = "400", description = "Dados inválidos fornecidos"),
                        @ApiResponse(responseCode = "401", description = "Usuário não autenticado")
        })
        public ResponseEntity<EssayResponse> createEssay(
                        @Valid @RequestBody @Parameter(description = "Dados da redação a ser criada", required = true) EssayCreateRequest request,
        ...
        @GetMapping("/{id}")
        @Operation(summary = "Buscar redação por ID", description = "Retorna uma redação específica do usuário autenticado, incluindo feedbacks")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Redação encontrada", content = @Content(schema = @Schema(implementation = EssayResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Redação não encontrada"),
                        @ApiResponse(responseCode = "401", description = "Usuário não autenticado")
        })
        public ResponseEntity<EssayResponse> getEssay(
                        @PathVariable @Parameter(description = "ID da redação", required = true, example = "1") Long id,
                        Authentication authentication) {
        ...
        @PutMapping("/{id}")
        @Operation(summary = "Atualizar redação", description = "Atualiza uma redação existente (apenas rascunhos podem ser editados)")
        @ApiResponses(value = {
        ...
        public ResponseEntity<EssayResponse> updateEssay(
                        @PathVariable @Parameter(description = "ID da redação", required = true) Long id,
        ...
        @DeleteMapping("/{id}")
        @Operation(summary = "Deletar redação", description = "Remove uma redação (redações já analisadas não podem ser deletadas)")
        @ApiResponses(value = {
        ...
        @GetMapping
        @Operation(summary = "Listar redações do usuário", description = "Retorna uma lista paginada das redações do usuário autenticado com filtros opcionais")
        @ApiResponses(value = {
        ...
        public ResponseEntity<Page<EssayResponse>> getUserEssays(
                        ...
                        @RequestParam(defaultValue = "updatedAt") @Parameter(description = "Campo para ordenação", example = "updatedAt") String sortBy,
                        @RequestParam(defaultValue = "desc") @Parameter(description = "Direção da ordenação", example = "desc") String sortDir,
                        @RequestParam(required = false) @Parameter(description = "Filtrar por status", example = "DRAFT") Essay.EssayStatus status,
                        @RequestParam(required = false) @Parameter(description = "Palavra-chave para busca no título, tema ou conteúdo", example = "educação") String keyword,
                ...
                User user = getCurrentUser(authentication);
                log.info("Buscando redações do usuário: {} - página: {}, status: {}, keyword: {}, date: {}",
                                user.getEmail(), page, status, keyword, date);
                ...
                Page<EssayResponse> response = essayService.getUserEssaysWithFilters(user, status, keyword, filterDate,
                                pageable);
                return ResponseEntity.ok(response);
        }

        @PostMapping("/{id}/submit")
        @Operation(summary = "Submeter redação para análise", description = "Envia uma redação para análise automática com IA. A redação deve estar em rascunho.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Reenviar redação para análise", description = "Reenvia uma redação para nova análise com IA. Permite reprocessar redações que estão em análise (SUBMITTED) ou já analisadas (ANALYZED). Útil quando a análise falhou ou o estudante deseja uma nova avaliação.")
        @ApiResponses(value = {
        ...
        @Operation(summary = "Buscar redações por status", description = "Retorna todas as redações do usuário com um status específico (método legado, use /essays com filtros)")
        @ApiResponses(value = {
        ...
        public ResponseEntity<List<EssayResponse>> getEssaysByStatus(
                        @PathVariable @Parameter(description = "Status das redações", required = true, schema = @Schema(implementation = Essay.EssayStatus.class)) Essay.EssayStatus status,
                        Authentication authentication) {
        ...
        @GetMapping("/status/{status}/filtered")
        @Operation(summary = "Buscar redações por status com filtros e paginação", description = "Retorna uma lista paginada das redações do usuário com um status específico, com filtros opcionais")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Lista paginada de redações por status")
        })
        public ResponseEntity<Page<EssayResponse>> getEssaysByStatusWithFilters(
                        @PathVariable @Parameter(description = "Status das redações", required = true) Essay.EssayStatus status,
                        @RequestParam(defaultValue = "0") @Parameter(description = "Número da página (0-indexed)", example = "0") int page,
                        @RequestParam(defaultValue = "10") @Parameter(description = "Tamanho da página", example = "10") int size,
                        @RequestParam(defaultValue = "updatedAt") @Parameter(description = "Campo para ordenação", example = "updatedAt") String sortBy,
                        @RequestParam(defaultValue = "desc") @Parameter(description = "Direção da ordenação", example = "desc") String sortDir,
                ...
                User user = getCurrentUser(authentication);
                log.info("Buscando redações do usuário: {} com status: {} - página: {}, keyword: {}, date: {}",
                                user.getEmail(), status, page, keyword, date);
                ...
                Page<EssayResponse> response = essayService.getUserEssaysByStatusWithFilters(user, status, keyword,
                                filterDate, pageable);
                return ResponseEntity.ok(response);
        }

        @GetMapping("/search")
        @Operation(summary = "Buscar redações por palavra-chave", description = "Busca redações do usuário que contenham a palavra-chave no título, tema ou conteúdo")
        @ApiResponses(value = {
        ...
        public ResponseEntity<Page<EssayResponse>> searchEssays(
                        @RequestParam @Parameter(description = "Palavra-chave para busca", required = true, example = "educação") String keyword,
